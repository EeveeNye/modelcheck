...
namespace Battlehub.RTHandles
{
    public interface IRuntimeSceneComponent : IRuntimeSelectionComponent
    {
        ...
        [Obsolete("Use CanRotate instead")] bool CanOrbit { get; set; }
        ...
        [Obsolete] float ZoomSpeed { get; set; }
    ...
    public class RuntimeSceneComponent : RuntimeSelectionComponent, IRuntimeSceneComponent
    {
        ...
        [SerializeField] private RectTransform m_sceneGizmoTransform = null;
        ...
        [SerializeField] private bool m_changeOrthographicSizeOnly = false;
        [SerializeField] private bool m_canRotate = true;
        [SerializeField] private bool m_canFreeMove = true;
        [SerializeField] private float m_orbitDistance = 5.0f;
        [SerializeField] private float m_boundingSphereRadius = 10000.0f;
        [SerializeField] private float m_minOrthoSize = 0.01f;
        [SerializeField] private float m_maxOrthoSize = 10000.0f;
        [SerializeField] private bool m_constantZoomSpeed = false;
        [SerializeField] private float m_zoomSensitivity = 1.0f;
        [SerializeField] private float m_moveSensitivity = 1.0f;
        [SerializeField] private float m_rotationSensitivity = 1.0f;
        [SerializeField] private float m_freeMovementSmoothSpeed = 10.0f;
        [SerializeField] private float m_freeRotationSmoothSpeed = 10.0f;
        [SerializeField] private bool m_rotationInvertX = false;
        [SerializeField] private bool m_rotationInvertY = false;
        ...
        [SerializeField] private SceneGizmo m_sceneGizmo;
        ...
        public override void Focus(FocusMode focusMode = FocusMode.Selected)
        {
            ...
            if (focusMode == FocusMode.Selected || focusMode == FocusMode.Default)
            {
                ...
                if ((Selection.activeTransform.gameObject.hideFlags & HideFlags.DontSave) != 0 ||
                    Selection.activeGameObject.IsPrefab())
                {
                    return;
                }
            ...
            {
                transforms = Editor.Object.Get(true).SelectMany(e => e.GetComponentsInChildren<Transform>())
        ...
        private void Focus(float distance, float objSize)
        {
            ...
            m_focusAnimation = new Vector3AnimationInfo(Window.Camera.transform.position,
                PivotTransform.position - distance * Window.Camera.transform.forward, duration,
            ...
            Run.Instance.Animation(new FloatAnimationInfo(m_orbitDistance, distance, duration,
                Vector3AnimationInfo.EaseOutCubic,
                (target, value, t, completed) => { m_orbitDistance = value; }));

            Run.Instance.Animation(new FloatAnimationInfo(Window.Camera.orthographicSize, objSize, duration,
                Vector3AnimationInfo.EaseOutCubic,
                (target, value, t, completed) =>
                {
                    if (Window.Camera)
        ...
        public virtual void Zoom(float deltaZ, Quaternion rotation, float epsilonSq)
        {
            ...
            m_targetPosition = cameraTransform.TransformVector(fwd) + cameraTransform.rotation * negDistance +
